            ...
            for (const items in cartItems) {
                for (const item in cartItems[items]) {
                    if (cartItems[items][item] > 0) {
                        tempData.push({
                            _id: items,  /* get id item when add to cart */
                            size: item,  /* set size item when add to cart cart */
                        ...
                        return (
                            <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                                <div className='flex items-start gap-6'>
                                    <img src={productData.image[0]} alt="" className='w-16 sm:w-20' /> {/* show image of item in cart */}
                                    <div>
                                        <p className='text-xs sm:text-lg font-medium'>{productData.name}</p> {/* show name of item in cart */}
                                        <div className='flex item-center gap-5 mt-2'>
                                            <p>{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(productData.price)}</p> {/* show price below item in cart */}
